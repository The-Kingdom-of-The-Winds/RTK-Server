...
static const int packet_len_table[] = {
	72, -1, 20, 24, -1, 6, 255, -1, 28 , sizeof(struct board_show_0) + 2, sizeof(struct boards_read_post_0) + 2 , 4 , sizeof(struct boards_post_0) + 2 , 4124 , 20 , 4124, 30, 255, 255, 255, 255, 255, //0x3000 - 0x3015
	0 }; //NULL terminator
...
int mapif_parse_userlist(int fd) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `ChaPthId`, `ChaMark`, `ChaClnId`, `ChaName`, `ChaHunter`, `ChaNation` FROM `Character` WHERE `ChaOnline` = 1 AND `ChaHeroes` = 1  GROUP BY `ChaName`, `ChaId` ORDER BY `ChaMark` DESC, `ChaLevel` DESC, SUM((`ChaBaseMana`*2) + `ChaBaseVita`) DESC, `ChaId` ASC")
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		...
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 4, SQLDT_INT, &hunter, 0, NULL, NULL)
...
int mapif_parse(int fd) {
	...
	case 0x300F: mapif_parse_nmailwritecopy(fd); break;
