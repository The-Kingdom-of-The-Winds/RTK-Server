...
bool bannedIPCheck(char* ip) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `BndId` FROM `BannedIP` WHERE `BndIP` = '%s'", ip)
...
int clif_accept(int fd) {
	...
	if (DDOS)
	{
		...
		// Add_Throttle( session[fd]->client_addr);
		Log_Add("BANNED", "<%02d:%02d> Banned IP detected from %s - ip throttled.\n", getHour(), getMinute(), ip);
...
int clif_sendurl(int fd, int type, char* url)
{
	...
	WFIFOB(fd, 3) = 0x66;
	WFIFOB(fd, 4) = 0x03;
	WFIFOB(fd, 5) = type; // type. 0 = ingame browser, 1= popup open browser then close client, 2 = popup
...
int reg_check(const char* n, int len) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `ChaId` FROM `Character` WHERE `ChaName` = '%s'", buf)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
...
int maintenance_mode() {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `MaintenanceMode` FROM `Maintenance`")
		|| SQL_ERROR == SqlStmt_Execute(stmt)
...
int maintenance_override(const char* n, int len) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `ChaGMLevel` FROM `Character` WHERE `ChaName` = '%s'", buf)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_INT, &nFlag, 0, NULL, NULL)
		)
	{
...
int clif_parse(int fd) {
	...
	case 0x02:
		...
		if ((RFIFOB(fd, 5) > 12) || (RFIFOB(fd, 5) < 3) || string_check_allchars(RFIFOP(fd, 6), RFIFOB(fd, 5))) {
	...
	case 0x26:
		...
		if (RFIFOB(fd, 7 + RFIFOB(fd, 5) + RFIFOB(fd, 6 + RFIFOB(fd, 5))) > 8 || RFIFOB(fd, 7 + RFIFOB(fd, 5) + RFIFOB(fd, 6 + RFIFOB(fd, 5))) < 3) {
			clif_message(fd, 0x05, login_msg[LGN_ERRPASS]);
			break;
		}
