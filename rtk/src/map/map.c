...
//map_freeblock_lock();	// ����������̉�����֎~���� - Prohibit release from memory
	...
	//map_freeblock_unlock();	// ����������� - Allow release
...
int map_loadregistry(int id) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `MrgIdentifier`, `MrgValue` FROM `MapRegistry` WHERE `MrgMapId` = '%d' LIMIT %d", id, MAX_MAPREG)
...
int map_lastdeath_mob(MOB* p) {
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `Spawns%i` SET `SpnLastDeath`='%u' WHERE `SpnX`='%u' AND `SpnY`='%u' AND `SpnMapId`='%u' AND `SpnId`='%u'", serverid, p->last_death, p->startx, p->starty, p->bl.m, p->id))
	{
...
int map_read() {//int id, const char *title, char bgm, int pvp, int spell, unsigned short light, unsigned short weather, unsigned int sweeptime, unsigned char cantalk, unsigned char showghosts, unsigned char region, unsigned char indoor, unsigned char warpout, const char *map_file) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `MapId`, `MapName`, `MapBGM`, `MapBGMType`, `MapPvP`, `MapSpells`, `MapLight`, `MapWeather`, `MapSweepTime`, `MapChat`, `MapGhosts`, `MapRegion`, `MapIndoor`, `MapWarpout`, `MapBind`, `MapFile`, `MapReqLvl`, `MapReqPath`, `MapReqMark`, `MapCanSummon`, `MapReqVita`, `MapReqMana`, `MapLvlMax`, `MapVitaMax`, `MapManaMax`, `MapRejectMsg`, `MapCanUse`, `MapCanEat`, `MapCanSmoke`, `MapCanMount`, `MapCanGroup`, `MapCanEquip` FROM `Maps` WHERE `MapServer` = '%d' ORDER BY `MapId`", serverid)
		...
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 3, SQLDT_USHORT, &bgmtype, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 4, SQLDT_UCHAR, &pvp, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 5, SQLDT_UCHAR, &spell, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 6, SQLDT_UCHAR, &light, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 7, SQLDT_UCHAR, &weather, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 8, SQLDT_UINT, &sweeptime, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 9, SQLDT_UCHAR, &cantalk, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 10, SQLDT_UCHAR, &show_ghosts, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 11, SQLDT_UCHAR, &region, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 12, SQLDT_UCHAR, &indoor, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 13, SQLDT_UCHAR, &warpout, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 14, SQLDT_UCHAR, &bind, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 15, SQLDT_STRING, &mapfile, sizeof(mapfile), NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 16, SQLDT_UINT, &reqlvl, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 17, SQLDT_UCHAR, &reqpath, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 18, SQLDT_UCHAR, &reqmark, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 19, SQLDT_UCHAR, &summon, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 20, SQLDT_UINT, &reqvita, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 21, SQLDT_UINT, &reqmana, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 22, SQLDT_UINT, &lvlmax, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 23, SQLDT_UINT, &vitamax, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 24, SQLDT_UINT, &manamax, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 25, SQLDT_STRING, &maprejectmsg, sizeof(maprejectmsg), NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 26, SQLDT_UCHAR, &canUse, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 27, SQLDT_UCHAR, &canEat, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 28, SQLDT_UCHAR, &canSmoke, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 29, SQLDT_UCHAR, &canMount, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 30, SQLDT_UCHAR, &canGroup, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 31, SQLDT_UCHAR, &canEquip, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;
	}
...
int map_reload() {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `MapId`, `MapName`, `MapBGM`, `MapBGMType`, `MapPvP`, `MapSpells`, `MapLight`, `MapWeather`, `MapSweepTime`, `MapChat`, `MapGhosts`, `MapRegion`, `MapIndoor`, `MapWarpout`, `MapBind`, `MapFile`, `MapReqLvl`, `MapReqPath`, `MapReqMark`, `MapCanSummon`, `MapReqVita`, `MapReqMana`, `MapLvlMax`, `MapVitaMax`, `MapManaMax`, `MapRejectMsg`, `MapCanUse`, `MapCanEat`, `MapCanSmoke`, `MapCanMount`, `MapCanGroup`, `MapCanEquip` FROM `Maps` WHERE `MapServer` = '%d'", serverid)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_STRING, &title, sizeof(title), NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 2, SQLDT_USHORT, &bgm, 0, NULL, NULL)
...
int boards_showposts(USER* sd, int board) {
	...
	if (sd->board_canwrite) {
		if (sd->board_canwrite == 6) a.flags = sd->board_canwrite; // special write flag added (to make client send packet when clicking "write")
		else a.flags |= BOARD_CAN_WRITE;
	}
...
int boards_post(USER* sd, int board) {
	...
	WFIFOHEAD(char_fd, sizeof(header) + 2); // 4001(Body) + 53(topic) + 2(sfd) + 2(ID) + 2(board) + 1(null-terminator)*2
...
int nmail_luascript(USER* sd, int to, int topic, int msg) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `Mail` (`MalChaName`, `MalChaNameDestination`, `MalBody`) VALUES ('%s', 'Lua', '%s')", sd->status.name, escape)) {
		Sql_ShowDebug(sql_handle);
		return 0;
	}
...
int nmail_poemscript(USER* sd, char* topic, char* message) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `BrdId` FROM `Boards` WHERE `BrdBnmId` = '19' AND `BrdChaId` = '%d'", sd->status.id)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &poemid, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;
	}
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT MAX(`BrdPosition`) FROM `Boards` WHERE `BrdBnmId` = '19'")
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &boardpos, 0, NULL, NULL))
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `Boards` (`BrdBnmId`, `BrdChaName`, `BrdChaId`, `BrdTopic`, `BrdPost`, `BrdMonth`, `BrdDay`, `BrdPosition`) VALUES ('19', '%s', '%d', '%s', '%s', '%d', '%d', '%u')", "Anonymous", sd->status.id, escape, escape2, month, day, boardpos + 1)) {
		Sql_ShowDebug(sql_handle);
...
int nmail_write(USER* sd) {
	...
	if (strcmpi(to_user, "poems") == 0 || strcmpi(to_user, "poem") == 0) {
		if (map_readglobalgamereg("poemAccept") == 0) { nmail_sendmessage(sd, "Currently not accepting poem submissions.", 6, 0); return 0; }
...
int nmail_sendmail(USER* sd, char* to_user, char* topic, char* message) {
	if (strlen(to_user) > 16 || strlen(topic) > 52 || strlen(message) > 4000) return 0;
...
int map_addmob(USER* sd, unsigned int id, int start, int end, unsigned int replace) {
	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `Spawns%d` (`SpnMapId`, `SpnX`, `SpnY`, `SpnMobId`, `SpnLastDeath`, `SpnStartTime`, `SpnEndTime`, `SpnMobIdReplace`) VALUES(%d, %d, %d, %d, %d, %d, %d, %d)", serverid, sd->bl.m, sd->bl.x, sd->bl.y, id, 0, start, end, replace))
		Sql_ShowDebug(sql_handle);
	return 0;
}
...
int map_loadgameregistry() {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `GrgIdentifier`, `GrgValue` FROM `GameRegistry%d` LIMIT %d", serverid, MAX_GAMEREG)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_STRING, &reg.str, sizeof(reg.str), NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_INT, &reg.val, 0, NULL, NULL)
		)
...
int map_savegameregistry(int i) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `GrgId` FROM `GameRegistry%d` WHERE `GrgIdentifier` = '%s'", serverid, p->str)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	if (save_id) {
		if (p->val == 0) {
			if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `GameRegistry%d` WHERE `GrgIdentifier` = '%s'", serverid, p->str)) {
				Sql_ShowDebug(sql_handle);
				return 0;
			}
		}
		else {
			if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `GameRegistry%d` SET `GrgIdentifier` = '%s', `GrgValue` = '%d' WHERE `GrgId` = '%d'", serverid, p->str, p->val, save_id)) {
				Sql_ShowDebug(sql_handle);
				return 0;
			}
		}
		...
		if (p->val > 0) {
			if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `GameRegistry%d` (`GrgIdentifier`, `GrgValue`) VALUES ('%s', '%d')", serverid, p->str, p->val)) {
				Sql_ShowDebug(sql_handle);
				return 0;
			}
		}
...
int map_loadclanbank(int id) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `CbkEngrave`, `CbkItmId`,`CbkAmount`,`CbkChaIdOwner`, `CbkPosition`, `CbkCustomLook`, `CbkCustomLookColor`, `CbkCustomIcon`, `CbkCustomIconColor`, `CbkProtected`, `CbkNote` FROM `ClanBanks` WHERE `CbkClnId` = '%u' LIMIT 255", id)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_STRING, &cbank.real_name, sizeof(cbank.real_name), NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_UINT, &cbank.item_id, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 2, SQLDT_UINT, &cbank.amount, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 3, SQLDT_UINT, &cbank.owner, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 4, SQLDT_UCHAR, &cbank.pos, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 5, SQLDT_UINT, &cbank.customLook, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 6, SQLDT_UINT, &cbank.customLookColor, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 7, SQLDT_UINT, &cbank.customIcon, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 8, SQLDT_UINT, &cbank.customIconColor, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 9, SQLDT_UINT, &cbank.protected, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 10, SQLDT_STRING, &cbank.note, sizeof(cbank.note), NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return -1;
	}
...
int map_saveclanbank(int id) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `CbkPosition` FROM `ClanBanks` WHERE `CbkClnId` = '%u' LIMIT 255", id)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
	...
	for (i = 0; i < max; i++) {
		...
		if (save_id[i] == i) {
			if (clan->clanbanks[i].item_id == 0) {
				if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `ClanBanks` WHERE `CbkClnId` = '%u' AND `CbkPosition` = '%d'", id, i)) {
					Sql_ShowDebug(sql_handle);
					return 0;
				}
			}
			else {
				if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `ClanBanks` SET `CbkItmId` = '%u', `CbkAmount` = '%u', `CbkChaIdOwner` = '%u', `CbkTimer` = '%u', `CbkEngrave` = '%s', `CbkCustomLook` = '%u', `CbkCustomLookColor` = '%u', `CbkCustomIcon` = '%u', `CbkCustomIconColor` = '%u', `CbkProtected` = '%d', `CbkNote` = '%s' WHERE `CbkClnId` = '%u' AND `CbkPosition` = '%d'",
					clan->clanbanks[i].item_id, clan->clanbanks[i].amount, clan->clanbanks[i].owner, clan->clanbanks[i].time, escape, clan->clanbanks[i].customLook, clan->clanbanks[i].customLookColor, clan->clanbanks[i].customIcon, clan->clanbanks[i].customIconColor, clan->clanbanks[i].protected, escape2, id, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
			...
			if (clan->clanbanks[i].item_id > 0) {
				if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `ClanBanks` (`CBkClnId`, `CbkItmId`, `CbkAmount`, `CbkChaIdOwner`, `CbkTimer`, `CbkEngrave`, `CbkCustomLook`, `CbkCustomLookColor`, `CbkCustomIcon`, `CbkCustomIconColor`, `CbkProtected`, `CbkNote`, `CbkPosition`) VALUES ('%u', '%u', '%u', '%u', '%u', '%s', '%u', '%u', '%u', '%u', '%d', '%s', '%d')",
					id, clan->clanbanks[i].item_id, clan->clanbanks[i].amount, clan->clanbanks[i].owner, clan->clanbanks[i].time, escape, clan->clanbanks[i].customLook, clan->clanbanks[i].customLookColor, clan->clanbanks[i].customIcon, clan->clanbanks[i].customIconColor, clan->clanbanks[i].protected, escape2, i))
				{
					Sql_ShowDebug(sql_handle);
					return 0;
				}
